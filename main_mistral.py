import json
import base64
import os
from datetime import datetime
from mistralai.client import Mistral 

client = Mistral(api_key=os.getenv("MISTRAL_API_KEY"))

def process_receipt(image_path):
    today_str = datetime.now().strftime("%Y-%m-%d")
    
    try:
        with open(image_path, "rb") as image_file:
            base64_image = base64.b64encode(image_file.read()).decode('utf-8')
        
        prompt = prompt = f"""
        【任務】你是一個專業的會計助手，請從這張收據/發票/帳單截圖中擷取消費資訊。
        
        【欄位嚴格定義】
        1. description: 商店名稱或主要消費類別。
        2. amount: 整張帳單的「最終付款總額」。必須是純數字。
        3. notes: 「純字串」。整理其他不屬於明細的備註。
        4. category: 請從「飲食」、「交通」、「住宿」、「娛樂」、「購物」、「一般支出」中選擇最適合的一項。
        5. date: 消費日期。請從圖片中尋找，格式為 YYYY/MM/DD。若找不到，請填入 {today_str}。
        6. currency: 幣別。且「只能」輸出 "TWD"、"JPY" 或 "USD"。(台灣發票請務必輸出 "TWD")
        7. items: 陣列。擷取每一項購買的商品明細，必須包含 name (商品名), price (該項總價, 純數字)。若無明細則回傳空陣列 []。
        
        【輸出規則】
        必須且只能回傳一個完全符合以下結構的 JSON 物件：
        {{
            "description": "商店名稱",
            "amount": 1000,
            "currency": "TWD",
            "date": "2026/07/17",
            "category": "飲食",
            "notes": "無其他備註",
            "items": [
                {{"name": "商品A", "price": 300}},
                {{"name": "商品B", "price": 700}}
            ]
        }}
        """

        response = client.chat.complete(
            model="pixtral-12b-2409",
            response_format={"type": "json_object"}, 
            messages=[
                {"role": "user", "content": [
                    {"type": "text", "text": prompt},
                    {"type": "image_url", "image_url": f"data:image/jpeg;base64,{base64_image}"}
                ]}
            ]
        )
        
        raw_text = response.choices[0].message.content.strip()
        data = json.loads(raw_text)
        
        try:
            data['amount'] = float(data.get('amount', 0))
        except:
            data['amount'] = 0
            
        if 'notes' in data and not isinstance(data['notes'], str):
            data['notes'] = json.dumps(data['notes'], ensure_ascii=False, indent=2)
            
        return data

    except Exception as e:
        return {"error": f"系統處理錯誤: {str(e)}"}